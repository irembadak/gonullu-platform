...
app = Flask(__name__)
# Sadece kendi frontend portuna izin vererek güvenliği artırabilirsin
CORS(app)

# Örnek Etkinlik Havuzu (Normalde DB'den veya bir modelden gelir)
EVENTS_DATABASE = [
...
def recommend():
    try:
        ...
        # AI Mantığı: Kullanıcıya en yakın olan ilk 3 etkinliği bul ve döndür
        # Mülakatta "Haversine Distance veya basit Öklid mesafesi ile sıralama yaptım" diyebilirsin.
        recommended = sorted(
            EVENTS_DATABASE, 
            key=lambda x: calculate_distance(user_lat, user_lng, x['lat'], x['lng'])
        )

        return jsonify(recommended[:3]) # En yakın 3 öneri

    except Exception as e:
        return jsonify({"error": str(e)}), 500

if __name__ == '__main__':
    # Flask'ı debug modda çalıştırmak geliştirmeyi kolaylaştırır
